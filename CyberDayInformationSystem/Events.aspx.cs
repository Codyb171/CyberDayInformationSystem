...
namespace CyberDayInformationSystem
{
    public partial class Events : System.Web.UI.Page
    {
        ...
        public void OldDataDisplay()
        {
            ...
            SqlConnection connection = new SqlConnection(cs);
            SqlCommand select =
                new SqlCommand("SELECT EVENTTITLE, EVENTDATE, STARTTIME, ENDTIME FROM EVENT1 WHERE EVENTID = @VALUE",
                    connection);
            SqlCommand select2 =
            ...
            if (EventFunctionSelection.SelectedValue == "2")
            {
                ...
                while (reader.Read())
                {
                    ...
                    NewStartTxt.Text = (reader["STARTTIME"].ToString());
                    NewEndTxt.Text = (reader["ENDTIME"].ToString());
                }
            ...
            if (TaskFunctionSelection.SelectedValue == "2")
            {
                ...
                while (reader.Read())
                {
                    ...
                    TaskNewStart.Text = reader["STARTTIME"].ToString();
                    TaskNewEnd.Text = reader["ENDTIME"].ToString();
                }
                ...
                if (CheckTask() == 0)
                {
                    ...
                    SqlCommand insert =
                        new SqlCommand(
                            ...
                            connection);
                    SqlCommand insert2 =
                        new SqlCommand("INSERT INTO EVENTITINERARY1 VALUES(@EVENT, @TASK)", connection);
                    SqlCommand select =
                        new SqlCommand(
        ...
        public int CheckEvent()
        {
            ...
            SqlCommand check =
                new SqlCommand(
                    "Select count(*) from EVENT1 where EVENTTITLE = @TITLE AND EVENTDATE = @DATE AND STARTTIME = @STARTTIME AND ENDTIME = @ENDTIME",
                    connection);
        ...
        protected void ModifyBut_Click(object sender, EventArgs e)
        {
            ...
            SqlCommand update =
                new SqlCommand(
                    "UPDATE FROM EVENT1 SET EVENTTITLE = @TITLE, EVENTDATE = @DATE, STARTTIME = @STARTTIME, ENDTIME = @ENDTIME WHERE EVENTID = @VALUE",
                    connection);
        ...
        protected void DelBut_Click(object sender, EventArgs e)
        {
            string cs = ConfigurationManager.ConnectionStrings["INFO"].ConnectionString;
            SqlConnection connection = new SqlConnection(cs);
            SqlCommand delete = new SqlCommand("DELETE FROM EVENTITINERARY1 WHERE EVENT = @VALUE", connection);
            SqlCommand delete2 = new SqlCommand("DELETE FROM EVENT1 WHERE EVENTID = @EVENTID", connection);
            string select = "SELECT (EVENTTITLE + '-' + EVENTDATE) AS EVENTINFO, EVENTID FROM EVENT1";
        ...
        protected void TaskDelBut_Click(object sender, EventArgs e)
        {
            string cs = ConfigurationManager.ConnectionStrings["INFO"].ConnectionString;
            SqlConnection connection = new SqlConnection(cs);
            SqlCommand delete = new SqlCommand("DELETE FROM EVENTITINERARY1 WHERE TASK = @VALUE", connection);
        ...
        public int getEXEventID()
        {
            ...
            string cs = ConfigurationManager.ConnectionStrings["INFO"].ConnectionString;
            SqlConnection connection = new SqlConnection(cs);
            SqlCommand command = new SqlCommand("Select MAX(EVENTID) from EVENT1", connection);
